...
public class Product {
    ...
    public static ObservableList<Product> getProductsByName(String productName) {
        String selectProducts = String.format("SELECT * FROM products WHERE pname LIKE '%%%s%%'", productName.toLowerCase());
        return getProductList(selectProducts);
    }
    ...
    private static ObservableList<Product> getProductList(String query) {
        ...
        try {
            ...
            while (rs.next()) {
                data.add(new Product(rs.getInt("pid"), rs.getString("pname"), rs.getDouble("price")));
